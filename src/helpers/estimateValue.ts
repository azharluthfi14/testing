...
const getWalletLocked = (wallet: Wallet): number => {
    return (Number(wallet.locked) || 0);
};
...
export const estimateValueTotal = (targetCurrency: string, currencies: Currency[], wallets: Wallet[], markets: Market[], marketTickers: MarketTicker): string => {
    const formattedTargetCurrency = targetCurrency.toLowerCase();
...
export const estimateValueBalance = (targetCurrency: string, currencies: Currency[], wallets: Wallet[], markets: Market[], marketTickers: MarketTicker): string => {
    const formattedTargetCurrency = targetCurrency.toLowerCase();
    let estimatedValue = 0;
    ...
    if (wallets && wallets.length) {
        for (const wallet of wallets) {
            ...
            if (formattedWalletCurrency === formattedTargetCurrency) {
                ...
                estimatedValue += estimateWithMarket(formattedTargetCurrency, formattedWalletCurrency, getWalletBalance(wallet), currencies, markets, marketTickers);
            } else {
                estimatedValue += estimateWithoutMarket(formattedTargetCurrency, wallet.currency, getWalletBalance(wallet), currencies, markets, marketTickers);
            }
        }
    }
    ...
    if (wallets && wallets.length) {
        for (const wallet of wallets) {
            ...
            if (formattedWalletCurrency === formattedTargetCurrency) {
                ...
                estimatedValue += estimateWithMarket(formattedTargetCurrency, formattedWalletCurrency, getWalletLocked(wallet), currencies, markets, marketTickers);
            } else {
                estimatedValue += estimateWithoutMarket(formattedTargetCurrency, wallet.currency, getWalletLocked(wallet), currencies, markets, marketTickers);
            }
        }
    }
